...
export const getStaticPaths: GetStaticPaths = async() => {
  const res = await getPostsUrl({ key: process.env.BLOGGER_API_KEY }).then(res => res.data.items);
...
export const getStaticProps: GetStaticProps = async(context: any) => {
  ...
  try {
    ...
    const data = await getPost(id, { fields: 'id,title,location(name,lat,lng),content,labels,published,updated,images,author(displayName,image),replies(totalItems)', key: process.env.BLOGGER_API_KEY }).then(res => res.data);
    return {
      props: {
        post: data
      },
